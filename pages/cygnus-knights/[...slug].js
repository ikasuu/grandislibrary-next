...
function ClassOverview({post, slug}) {
    ...
    return (
        <div>
            <Head>
              <title>{`${data.class} Class Overview | MapleStory | Grandis Library`}</title>
...
export async function getStaticPaths() {
  ...
  const paths = classes.cygnusKnights.map((job) => ({
    params: { slug: [job[0]] },
  }));
...
export async function getStaticProps({ params }) {
  const res = await fetch(`https://raw.githubusercontent.com/ikasuu/grandislibrary/master/public/data/cygnus-knights/${params.slug[0]}.json`);
