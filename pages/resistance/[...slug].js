...
export async function getStaticPaths() {
  ...
  const paths = classes.resistance.map((job) => ({
    params: { slug: [job[0]] },
  }));
...
export async function getStaticProps({ params }) {
  const res = await fetch(`https://raw.githubusercontent.com/ikasuu/grandislibrary/master/public/data/resistance/${params.slug[0]}.json`);
