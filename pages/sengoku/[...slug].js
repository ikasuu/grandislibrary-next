...
function ClassOverview({post, slug}) {
    ...
    return (
        <div>
            ...
            <Container>
              <Alert variant="info">Sengoku Class Overviews have not yet been updated with remaster changes. Please check back later for updates</Alert>
...
export async function getStaticPaths() {
  ...
  const paths = classes.sengoku.map((job) => ({
    params: { slug: [job[0]] },
  }));
...
export async function getStaticProps({ params }) {
  const res = await fetch(`https://raw.githubusercontent.com/ikasuu/grandislibrary/master/public/data/sengoku/${params.slug[0]}.json`);
